 ...
 */
double clamp(double val1, double min, double max) {
	if (val1 > max) {
		return max;
	}
	if (val1 < min) {
...
uint8_t MiniPID::getMode() {
	return this->mode;
}

void MiniPID::setMode(uint8_t mod) {
	this->mode = mod;
	this->output = 0;
}

void MiniPID::compute() {
	this->getOutput();
}

double MiniPID::GetKp() {
	return this->P;
}

double MiniPID::GetKd() {
	return this->D;
}

double MiniPID::GetKi() {
	return this->I;
}

void MiniPID::setTunings(double p, double i, double d) {
	this->P = p;
	this->I = i;
	this->D = d;
}
